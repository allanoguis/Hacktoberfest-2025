...
export const GameService = {
    ...
    async getLeaderboard(limit = 10, offset = 0) {
        try {
            // Use the new database function for optimized performance
            const { data, error } = await supabase
                .rpc('get_leaderboard', { 
                    limit_count: limit, 
                    offset_count: offset 
            ...
            if (error) {
                ...
                // Fallback to the original method if function doesn't exist
            ...
            // Process avatar URLs with fallback logic
            const processedData = (data || []).map(player => ({
                ...player,
                profileImageUrl: player.profileImageUrl || player.profile_image_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${player.playerName || 'Guest'}`
            }));
    ...
    async getLeaderboardFallback(limit = 10, offset = 0) {
        ...
        // Filter for unique players and stop when we have the requested count
        const topPlayers = [];
        const seenPlayers = new Set();

        for (const game of games || []) {
            if (topPlayers.length >= limit) break;
            ...
            if (!seenPlayers.has(playerId)) {
                ...
                topPlayers.push({
                    ...
                    score: game.score,
                    time: typeof game.time === 'string' ? (new Date(game.time).getTime() / 1000) : game.time,
                    ...
                    gamesPlayed: 1 // Placeholder - would need additional query
                });
            }
        }
